...
public class ReasoningService extends ServiceAdapter{
	...
	private Response searchTripleFromGraph(String triple) {
		...
		//Populate response with history of triple
		graphElem.setAttribute("json", ServicesUtils.searchTripleOnGraph(triple, reasonerFacade.getInputOutputHandler().getAlternativeGraph()));
		return response;
	}
	...
	private Response getJsonGraph() {
		...
		graphElem.setAttribute("listOfVertices",ServicesUtils.getListofVertexJsonGraph(reasonerFacade.getInputOutputHandler().getAlternativeGraph()).toJSONString());
		return response;
	...
	private Response removeTriple() {
		...
		try {
            //Project<? extends RDFModel> project = ProjectManager.getCurrentProject(); // old
            //baseRDFTripleModel = project.getOntModel();   // old
            baseRDFTripleModel = getOntModel(); 
	...
	private Response startReasoning(String cycleReasoning, String idsRule,String output) {
		...
		try {
			...
			//check if project exists. If true do reasoning, else return a warning message
			if (project  == null){
				response = createReplyResponse(RepliesStatus.warning);
				Element dataElement = response.getDataElement();
				Element errorElem = XMLHelp.newElement(dataElement, "startReasoning");
				errorElem.setAttribute("warning", "You must select a project");
				return response;
			}
			...
			if (new_triple != null && new_triple.size() == 0 && reasonerFacade.getInputOutputHandler().getReasoningOutput().getInferenceRuleOutput().size() <= 0){
				...
				warnElem.setAttribute("warning", "No new triple has been inferred");
				return response;				
			}
            else {
			//Create response
			...
			try {
				String fileName = project.getName()+GraphServices.JSON_GRAPH_EXTENSION;
				FileWriter file = new FileWriter(GraphServices.JSON_GRAPH_FILE_PATH_DIRECTORY+"/"+fileName);
				file.write(ServicesUtils.convertGraphToJson(reasonerFacade.getInputOutputHandler().getAlternativeGraph()).toJSONString());
				file.flush();
				file.close();
