...
import it.uniroma2.reasoner.utils.Sanitazer;
...
public class Reasoner {
    ...
    public List<ARTStatement> reasoning() throws UnsupportedQueryLanguageException, ModelAccessException, MalformedQueryException, QueryEvaluationException, ModelUpdateException{
        ...
        //inferred triples
        //TODO instead of setting a big number, maybe it is better to use a boolean value
        ...
        for(count = 0;count <numberOfExecution;count++){
            ...
            for(InferenceRule inferenceRule: inferenceRules.getInferenceRules() ){
                ...
                //For each output that has been generated, check if the new information is already on the 
                // ontology, if false create new output with
                ...
                for(InferenceRuleQueryResult inferenceRuleQueryResult:inferenceRuleQueryResults ){
                    ...
                    //Check if a type rule is a consistency check rule
                    if(inferenceRule.getType().equals(InferenceRule.TPYE_INCONSISTENCY)){

                        InferenceRuleOutput inferenceRuleOutput = produceOutput(produceOutput, 
                        		inferenceRuleQueryResult, inferenceRule, 
                        		inputOutputHandler.getAlternativeGraph(), 
                        		inputOutputHandler.getAlternativeGraph());
                    ...
                    // the rule is not a consistency check rule
                    else if( !isStatementInTheOntology(inferenceRuleQueryResult, baseRDFTripleModel)){
                        ...
                        InferenceRuleOutput inferenceRuleOutput = produceOutput(produceOutput, 
                        		inferenceRuleQueryResult, inferenceRule, inputOutputHandler.getGraph(), 
                        		inputOutputHandler.getAlternativeGraph());
                        //Add the output on the list of output
                        inferenceRulesOutuput.add(inferenceRuleOutput);
                    }
            ...
            //Check if reasoner has to continue to iterate. If no new triple has been discovered, break the iteration
            if(outputRules <= 0){
    ...
    private boolean checkInconsistency(InferenceRuleOutput inferenceRuleOutput) {
        ...
        for(ARTStatement triple: inferenceRuleOutput.getFromTriple()){
        	...
        	String subject = Sanitazer.sanitizeURI(triple.getSubject().asURIResource());
    		String predicate = Sanitazer.sanitizeURI(triple.getPredicate().asURIResource());
    		String object =  Sanitazer.sanitizeURI(triple.getObject().asURIResource());
    ...
    /**
     * Check if statement is contained in the ontology. If true, add the statement in the ontology and return 
     ...
     * @throws ModelUpdateException
     */
    private boolean isStatementInTheOntology(InferenceRuleQueryResult inferenceRuleQueryResult, 
    		BaseRDFTripleModel model) throws ModelAccessException, ModelUpdateException {
