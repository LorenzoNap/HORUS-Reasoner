...
public class StartReasonerFacade {
	 ...
	 */
	public List<ARTStatement> startReasoner(BaseRDFTripleModel inputOntology, File inferenceRuleFile) 
			throws UnsupportedRDFFormatException, FileNotFoundException, IOException, GrammarException, 
			ValidateException, UnsupportedQueryLanguageException, ModelAccessException, 
		...
		ParseInferenceRulesFacade parseInferenceRulesFacade = new ParseInferenceRulesFacade(
				configurationParameter,inferenceRuleFile);
		...
		newInferredTriples = reasonerFacade.doHandler(inferenceRules,configurationParameter, 
				inputOutputHandler,inputOntology);
	...
	public void searchEdge(ARTStatement artStatement){
		...
		String subject = Sanitazer.sanitizeURI(artStatement.getSubject().asURIResource());
		String predicate = Sanitazer.sanitizeURI(artStatement.getPredicate().asURIResource());
		String object =  Sanitazer.sanitizeURI(artStatement.getObject().asURIResource());
		...
		if (inputOutputHandler.getGraph().containsVertex(vertexSource)){
			...
			Graph<String,String> graph = 
					GraphUtils.BreabreadthFirstSearch(inputOutputHandler.getGraph(),vertexSource);
